...
OctoGuide doesn't support custom (user-defined) rules yet: [🚀 Feature: Custom rules #22](https://github.com/OctoGuide/bot/issues/22).
...
- `about`: [`RuleAbout`](/generated/api/interfaces/ruleabout/): Metadata about the rule.
...
- `<entity>`: one of `comment`, `discussion`, `issue`, `pr`, or the catch-all `text`
  - It's omitted if every entity with that `<area>` is checked, as in `title-meaningful`
...
This will be included in any [rule reports](/generated/api/interfaces/rulereport/).

At least the properties in [`RuleAbout`](/generated/api/interfaces/ruleabout/) are required.
...
- `context`: [`RuleContext`](/generated/api/interfaces/rulecontext/)
