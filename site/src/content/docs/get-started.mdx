...
OctoGuide by default skips running rules for collaborator-created entities (issues, pull requests, and discussions)
and comments. This is useful because repository collaborators and owners typically
have a deeper understanding of the project's contribution guidelines that external
contributors may not have. Collaborators may also need flexibility to make changes
outside of the OctoGuide rules.

Collaborator detection uses the `author_association` field from GitHub's webhook payload.
Users with any of the following associations are considered collaborators:

- `COLLABORATOR` - Repository collaborator
- `OWNER` - Repository owner

Contributors with associations like `MEMBER` (organization member),
...
or `NONE` (no association) will still have rules enforced.

If you want OctoGuide to run rules on collaborator-created entities and comments,
you can enable this by setting `include-collaborators` to `"true"`:
