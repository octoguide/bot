...
:::note
Every rule supports the `include-associations` and `include-bots` options,
which override the equivalent top-level inputs for that one rule.
:::
...
You can enable, disable, and configure specific rules by providing a `rules` input set to a string equivalent to a JSON object.
Its keys should be the kebab-case rule names.
Its values should be booleans to enable or disable those rules, or objects of options to run those rules with.
...
Rule names use kebab-case and correspond to the rule names shown in the [Rules](/configs#rules-table) documentation.
When a rule is explicitly set via input, it takes precedence over the preset config settings.

#### Rule Options

Setting a rule to an object both enables the rule and gives it options to run with.
Every rule supports the `include-associations` and `include-bots` options,
which override the equivalent top-level inputs for that one rule.
`include-associations` is provided per-rule as an array rather than a comma-separated string.

**Example:**

<Code
	code={getStartedRuleOptions}
	meta="lang=yml"
	lang="diff"
	title=".github/workflows/octoguide.yml"
/>

Some rules declare their own default options.
Those defaults take precedence over the top-level `include-associations` and `include-bots` inputs,
so a rule such as [`text-image-alt-text`](/rules/text-image-alt-text) that applies to all author associations
keeps doing so until you give it options of your own.
